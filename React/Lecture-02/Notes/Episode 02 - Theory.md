...
## Q: What and how `node-modules` works and also explain `transitive-dependencies` ? 
A: Node modules ==> Fetches all the code of all the dependencies in our system.It is kind of a database where all our packages exists.
When we install any particular package, in node modules there are several other packages installed also with the package that we want to install , this is because of transitive dependencies which means a package can depend on other package , and that package can depend on some other package, so that's why node modules is a heavy folder , it installs all the dependencies, those package that we want and those also on which this package depends and so on.... 